...
const ProductImportModal = ({ isOpen, onClose, onSuccess, adminCompany }) => {
  ...
  async function processFile(f) {
    ...
    try {
      ...
      const { valid, errors: errs, warnings: warns } = validateProductRows(parsed, groupNames);
  ...
  async function handleImport() {
    ...
    for (let i = 0; i < validRows.length; i += chunkSize) {
      ...
      const insertData = chunk.map(row => ({
        ...
        material_subgroup:    row.material_subgroup || null,
      ...
      const { data, error } = await supabase
        .from("products")
        .upsert(insertData, { onConflict: "company_id,material", ignoreDuplicates: false })
        .select("id");
      ...
      <div className="bg-card border border-border rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold">Import Products</h3>
        ...
        <div className="flex-1 overflow-auto px-6 py-5">
                    ...
                    <table className="w-full text-xs">
                      <thead className="bg-red-50 dark:bg-red-950/20">
                        <tr>
                          ...
                          <th className="px-3 py-2 text-left text-red-700">Item Code</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-red-100">
                        {errors.map((e, i) => (
                          <tr key={i} className="bg-red-50/50 dark:bg-red-950/10">
                            ...
                            <td className="px-3 py-1.5 text-muted-foreground">{e.data?.material || "—"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                      ...
                      <tbody className="divide-y divide-border">
                        {validRows.slice(0, 10).map((r, i) => (
                          <tr key={i} className="hover:bg-accent">
                            ...
                            <td className="px-3 py-1.5">{r.material_group}</td>
                            <td className="px-3 py-1.5">{r.base_unit_of_measure}</td>
                            <td className="px-3 py-1.5">{r.unit_price ?? "—"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          {step === "importing" && (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              ...
              <p className="text-sm font-medium">
                Importing products for {adminCompany?.name}…
              </p>
